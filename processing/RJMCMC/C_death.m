function [nc,ind_zc,zc,xc,p_xc]=C_death(nc,ind_zc,zc,xc,p_xc,x_min,x_max,q,pdf,coord,cov_tune,Tem,i_c)
global acc attempt
...
C=2.38^2;
% C=2.38^2*cov_tune(ind_zc(ind),ind_zc(ind));

log_pro=log_normpdf(x_death,x_new_at_zd,sqrt(C));
log_pri=log(x_max-x_min)+log(nc)-log(21);
