...
n_min=R(1,1);n_max=R(1,2);x_min=R(2,1);x_max=R(2,2);coord=linspace(0,l,nn);
...
for t=2:tune
    for i=1:N
        [Nc(i),ind_Zc(:,i),Zc(:,i),Xc(:,i),p_Xc(i)]=moveRJ(Nc(i),ind_Zc(:,i),Zc(:,i),Xc(:,i),p_Xc(i),n_min,n_max,x_min,x_max,q,pdf,coord,1,t,tune,Tem(i),i);
...
for t=tune+1:T
    for i=1:N
        ...
        if move_mode==1
            ...
            [cov_tune(:,:,i),mean_tune(:,:,i)]=ite_C(cov_tune(:,:,i),mean_tune(:,:,i),load_c,attempt(3,i));
        end
    end
    n(t)=Nc(1);z(t,:)=Zc(:,1)';x(t,:)=Xc(:,1)';p_x(t)=p_Xc(1);
