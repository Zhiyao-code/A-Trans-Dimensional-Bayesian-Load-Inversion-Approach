function [n,z,x,p_x]=AP_PT_RJMCMC(pdf,Range,T,nn,N)
global attempt acc s_att s_acc 
attempt=ones(3,N);acc=ones(3,N);s_att=zeros(1,N); s_acc=zeros(1,N);

n_min=Range(1,1);n_max=Range(1,2);z_min=Range(2,1);z_max=Range(2,2);x_min=Range(3,1);x_max=Range(3,2);
coord=linspace(z_min,z_max,nn+1);coord=coord(1:end-1);
